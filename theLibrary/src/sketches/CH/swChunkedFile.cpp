...
CswChunkedFile::CswChunkedFile(void)
{
...
BOOL CswChunkedFile::InitChunked (BOOL fInitIfNew)
{
	CHAR rgchSig[4];
	CHAR rgchTrueSig[] = "SWCH";
	SeekBegin ();
	if (Read (rgchSig, 4) != EOF)
	{
		if (memcmp(rgchSig, rgchTrueSig, 4) != 0)
			return FALSE;
	}
	else
	{
		// seems to be new file. Trying to write 
		if (fInitIfNew && (Write (rgchTrueSig, 4) != 4))
			return FALSE;
	}
